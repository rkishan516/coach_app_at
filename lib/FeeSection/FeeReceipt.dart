...
class FeeReceipt extends StatefulWidget {
  ...
  FeeReceipt({this.isEdit, this.courseid});
  @override
...
class _FeeReceiptState extends State<FeeReceipt> {
  ...
  final TextEditingController _fineDurationText = TextEditingController(text: "0");
  final TextEditingController _ddText = TextEditingController();
  final TextEditingController _yyText = TextEditingController();
  var _mmperiod = ["MM","01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "11", "12"];
  ...
  _changeTotalFees(value) {
    ...
    setState(() {
      _totalText.text = (
              double.parse(_admissionText.text != '' ? _admissionText.text : "0") +
  ...
  _saveintodatabase() {
    dbRef
        .reference()
        .child("institute/${FireBaseAuth.instance.instituteid}/branches/${FireBaseAuth.instance.branchid}/courses/410665312")
        .child("fees")
        .update({
      "FeeSection": {
     ...
     if(!toggleValue1){
     dbRef.reference().child("institute/${FireBaseAuth.instance.instituteid}/branches/${FireBaseAuth.instance.branchid}/courses/410665312/fees").child("MaxInstallment")
                 .update({
                   "IsMaxAllowed" : false
                 });
    dbRef.reference().child("institute/${FireBaseAuth.instance.instituteid}/branches/${FireBaseAuth.instance.branchid}/courses/410665312/fees").child("MaxInstallment/Installments")
                 .remove();                 
     }
      ...
      child: Row(
        ...
        children: [
          ...
          GestureDetector(
            onLongPress: () {
              if (toggleValue1){
               Navigator.push(context,MaterialPageRoute(builder: (context)=>Installments(_totalText.text,toggleValue1)));
            }
            },
 ...
 Widget _attachOneTimePay() {
              ...
              if (toggleValue3)
                showErrorDialog3(context).then((value) {
                  if (value == "ok") {
                    setState(() {
                      if (_ddText.text == "0" ||
                          _yyText.text == "0") toggleValue3 = false;
                    });
                  }
                });
              ;
  ...
  _loadFromDatabase() async {
    dbRef
        .reference()
        .child(
            "institute/${FireBaseAuth.instance.instituteid}/branches/${FireBaseAuth.instance.branchid}/courses/${widget.courseid}").child("fees").once().then((snapshot) {
      ...
      setState(() {
        ...
        toggleValue1 = map["MaxInstallment"]["IsMaxAllowed"] == true ? true : false;
        toggleValue2 = map["SetFine"]["IsFineAllowed"] == true ? true : false;
        toggleValue3 = map["OneTime"]["IsOneTimeAllowed"] == true ? true : false;
        _fineDurationText.text = map["SetFine"]["Duration"].toString().split(" ")[0];
        _currentFineDurationSelected = map["SetFine"]["Duration"].toString().split(" ")[1];
 ...
 toogleButton1() {
    setState(() {
      ...
      if (toggleValue1 && !isEdit) {
        Navigator.of(context).push(MaterialPageRoute(builder: (context)=>Installments(_totalText.text, dup))).then((value) {
          if (value == "confirm") {
            setState(() {
                toggleValue1 = true;
                print("iii");
              ...
              child: Row(
                    ...
                    Expanded(
                      flex: 1,
                                        child: DropdownButton<String>(
                              items:
                                  _mmperiod.map((String dropDownStringitem) {
                                return DropdownMenuItem<String>(
                                  value: dropDownStringitem,
                                  child: Text(dropDownStringitem),
                                );
    ...
    setState(() {
      ...
      if (toggleValue3 && !isEdit) {
        showErrorDialog3(context).then((value) {
          if (value == "ok") {
            setState(() {
              if (_yyText.text == "" || _ddText.text == ""||_mmSelected=="MM")
                toggleValue3 = false;
            });
          }
          else{
