...
class StudentModel{
  ...
  String totalfees;
  StudentModel(this.uid,this.address, this.acaedmicYear, this.courseName, this.discount, this.name,this.phoneNo, this.email, this.photoURL,  this.lastpaidInstallment, this.totalfees);
  ...
  lastpaidInstallment= value["course"][courseID]["fees"]["Installments"]["LastPaidInstallment"]; 
  allowedthrough = value["course"][courseID]["fees"]["Installments"]["AllowedThrough"];
  totalFees= fineMap["FeeSection"]["TotalFees"];
  map= value["course"][courseID]["fees"]["Installments"]; 

   _createInstallmentmodel(map, [], fineMap,discount).then((value) => listInstallment= value);
...
class NoInstallments{
  ...
  String totalfees;
  NoInstallments(this.sequence, this.amount, this.duration, this.fine, this.status, this.paidTime);
...
Future<List<NoInstallments>> _createInstallmentmodel( Map installments, List<NoInstallments> _listInstallment, Map fineMap, String discount ) async {
           ...
           installments.forEach((key, value) { 
             ...
             if(key!="AllowedThrough" && key!="LastPaidInstallment"){
              ...
              if(value["Status"]=="Paid"){
                 _listInstallment.add(NoInstallments(key, value["Amount"],value["Duration"],value["Fine"],value["Status"], value["PaidTime"]));
              }
              else{
                   String status="";
                   String fine="";
                  ...
                  fine= value["Fine"];
                      ...
                      int count= int.parse(fineMap["SetFine"]["Duration"].toString().split(" ")[0]);
                      String period= fineMap["SetFine"]["Duration"].toString().split(" ")[1];
                      
                      int durationinDays= period=="Day(s)"? count: period=="Month(s)"? count*30: count*365; 
                      
                        
                        double val= double.parse(( (difference/durationinDays).ceil() * double.parse(fineMap["SetFine"]["FineAmount"]) ).toStringAsFixed(2));
                        fine= val.toString();
                        status= "Fine";
                ...
                _listInstallment.add(NoInstallments(key, amountPass,value["Duration"],fine,status, value["PaidTime"]));
                  }
