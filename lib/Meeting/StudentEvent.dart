 ...
 void storeTokenintoDatabase() async {
    _firebaseMessaging.getToken().then((token){
       ...
       print("........$token.............");
        ...
        dbref.child('institute/${FireBaseAuth.instance.instituteid}/branches/${FireBaseAuth.instance.branchid}').child('admin').child(FireBaseAuth.instance.user.uid).update({
  ...
  _initializeevent() {
   ...
   Future<bool> checkentry(Event event) async {
     ...
     if(hostprevilage==4){
       if(type=="SubAdmins" && previlagelevel==3 && !leftuids.contains(FireBaseAuth.instance.user.uid))
         return true;
       else if(type=="MidAdmins" && previlagelevel==34 && !leftuids.contains(FireBaseAuth.instance.user.uid))  
         return true;
       else if(type=="SubAdmins within a MidAdmin" && (previlagelevel==34 || previlagelevel==3) && !(firstselecteduids=="") ){
          if(previlagelevel==34 && firstselecteduids.contains(FireBaseAuth.instance.user.uid))
          return true;
          else if(previlagelevel==3){
           DataSnapshot _snapshot =  await dbRef.reference().child('institute/${FireBaseAuth.instance.instituteid}/midAdmin').child(firstselecteduids.split(":_:_:")[0]).child("branches").once();
           String branches = _snapshot.value.toString();
           if(branches.contains(FireBaseAuth.instance.branchid) && !leftuids.contains(FireBaseAuth.instance.user.uid)) 
           return true;
           else 
           return false;
          }
       ...
       else if(type=="Authorities of a branch" && (previlagelevel==2 || previlagelevel==3) && firstselecteduids.contains(FireBaseAuth.instance.branchid) && !leftuids.contains(FireBaseAuth.instance.user.uid))
       return true;
       else
         return false;    
        ...
        if(type == "Other MidAdmins" && previlagelevel== 34 && !leftuids.contains(FireBaseAuth.instance.user.uid) )
          return true;
        else if(type=="SubAdmins" && previlagelevel==3)  {
          DataSnapshot _snapshot =  await dbRef.reference().child('institute/${FireBaseAuth.instance.instituteid}/midAdmin').child(hostuid).child("branches").once();
          String branches = _snapshot.value.toString();
          if(branches.contains(FireBaseAuth.instance.branchid) && !leftuids.contains(FireBaseAuth.instance.user.uid))
          return true;
          else 
       ...
       if(type=="Teachers" && previlagelevel==2 && !leftuids.contains(FireBaseAuth.instance.user.uid))
          return true;
       else if(type=="All students" && previlagelevel==1 && !leftuids.contains(FireBaseAuth.instance.user.uid))
          return true;
       else if(type=="Teachers of a course" &&previlagelevel==2 ){
      DataSnapshot _snapshot =  await dbRef.reference().child('institute/${FireBaseAuth.instance.instituteid}/branches/${FireBaseAuth.instance.branchid}/teachers/${FireBaseAuth.instance.user.uid}/courses').once();
      List list= _snapshot.value;
      list.forEach((element) { 
        if(firstselecteduids.contains(element["id"]) && !leftuids.contains(FireBaseAuth.instance.user.uid))
        return true;
        else
        return false;
      });
       ...
       DataSnapshot _snapshot =  await dbRef.reference().child('institute/${FireBaseAuth.instance.instituteid}/branches/${FireBaseAuth.instance.branchid}/courses/$courseid/subjects').orderByChild("mentor/${email.split("@")[0]}").once();
       if(_snapshot.value!=null && !leftuids.contains(FireBaseAuth.instance.user.uid))
       return true;
       else 
       return false;
              ...
              leading: IconButton(
                ...
                onPressed: (){
                  ...
                  Navigator.push(context, MaterialPageRoute(builder: (context) {
                    return VideoConferencing(room: _allEvent[index].title, subject: _allEvent[index].description,hostprevilagelevel: _allEvent[index].hostPrevilage, hostuid: _allEvent[index].hostuid, );
                  }));
                  }
