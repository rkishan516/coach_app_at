...
class _SelectCandidateState extends State<TwoStepSelectCandidate> {
  ...
  _loadFromDatabase() async {
   ...
   map.forEach((key, value) { 
     if(key.toString().length>=25){
         _list.add(TwoStepSelectedCandidateModel( value["branches"],value["district"], value["name"],value["photoUrl"], key ));
   ...
   mapofbranches.forEach((key, value) { 
         ...
         _list.add(TwoStepSelectedCandidateModel(key,value["address"], value["name"],"https://www.leafstudio.co.uk/wp-content/uploads/2019/11/person-icon-silhouette-png-0.png", key ));
         if(widget.firstselecteduids.contains(key))
           _correspondingmap[key]= true;
           else
           _correspondingmap[key]= false;
  ...
  //String _branches="[1515, 1901, 1902, 1404, 1301, 1215, 1101, 1704, 1812, 2014, 2101, 2201, 2301]";
   ...
   _branchlist.forEach((element) async { 
      DataSnapshot addresssnapshot = await dbref.reference().child('institute/${FireBaseAuth.instance.instituteid}/branches/${element.trim()}/address').once();
      DataSnapshot namesnapshot = await dbref.reference().child('institute/${FireBaseAuth.instance.instituteid}/branches/${element.trim()}/name').once();    
           ...
           _list.add(TwoStepSelectedCandidateModel(element.trim(),addresssnapshot.value, namesnapshot.value, "https://www.leafstudio.co.uk/wp-content/uploads/2019/11/person-icon-silhouette-png-0.png", element.trim()));
   ...
   DataSnapshot snapshot = await dbref.reference().child('institute/${FireBaseAuth.instance.instituteid}/branches/${FireBaseAuth.instance.branchid}/coursesList').once();
   Map map= snapshot.value;
   map?.forEach((key, value) { 
          _list.add(TwoStepSelectedCandidateModel( key,"", value ,"https://banner2.cleanpng.com/20180505/cre/kisspng-book-cover-outline-clip-art-5aed58b656e528.2169014815255041823559.jpg", key ));
         if(widget.firstselecteduids.contains(key))
           _correspondingmap[key]= true;
           else
           _correspondingmap[key]= false;
          ...
          itemBuilder:(context, index){
            return ListTile(
              ...
              onLongPress: (){
                ...
                Navigator.of(context).push(MaterialPageRoute(builder: (context)=>AfterSelectCandidate(leftuids: _leftuids, type: widget.type,searchVariable: _list[index].branchaddress, map: mapofbranches,))).
                       then((value){
                ...
                onChanged:(bool value){
                  setState(() {
                    ...
                    if(value){
                      ...
                      _firstselecteduids  = _firstselecteduids + _list[index].uid + ":_:_:"; 
                       Navigator.of(context).push(MaterialPageRoute(builder: (context)=>AfterSelectCandidate(leftuids: _leftuids, type: widget.type,searchVariable: _list[index].branchaddress, map: mapofbranches,))).
                       then((value){
                        _objectPass.leftuids= value;
                       });
                     ...
                     _firstselecteduids=_firstselecteduids.split(_list[index].uid+":_:_:").join();
                  });
                }
                ),
