...
class _InsideSheetState extends State<InsideSheet> {
  ...
  String stringjson=await DefaultAssetBundle.of(context).loadString("assets/Routing.json");
      ...
      _routejson.forEach((key, value) async{
        if( text.trim().toLowerCase().contains( key)){
          if(value.toString().startsWith("/show dialog")){
            ...
            showDialog(
                        context: context,
                        builder: RouteMap().createroute()[value]);
          }
          else{
          Navigator.of(context).pop();
        ...
        appBar: new AppBar(
          ...
          actions: [
            new PopupMenuButton<Language>(
              onSelected: (Language lang) { setState(() => selectedLang = lang);},
                  ...
                  Row(
                   ...
                   _buildButton(
                    onPressed: _isAvailable && !_isListening
                        ? () => _speech.listen(locale: selectedLang.code) .then((result) => print('_MyAppState.start => result ${result}')).catchError((err)=>{
                          print(err)
                        })
                        : _isListening ? () =>_speech.stop().then((result) => setState(() => _isListening = result)) : null,
                    label: _isListening
                        ? 'Listening...'
