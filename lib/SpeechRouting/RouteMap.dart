...
class RouteMap{
 ...
 Map<String , WidgetBuilder> createroute(){
   Map<String , WidgetBuilder> map ={
      ...
      '/all courses' : (context) => AllCoursePage(ref: FirebaseDatabase.instance.reference().child('/institute/${FireBaseAuth.instance.instituteid}/branches/${FireBaseAuth.instance.branchid}')),
      '/my courses' : (context) => CoursePage(),
      '/fees section' : (context) => CoursePage(isFromDrawer: true,), 
      '/admin corner' :  (context) => NoticeBoard(totalNotice: GuruCoolDrawerState().totalNotice, totalPublicContent: GuruCoolDrawerState().totalPublicContent,),                
      '/onetime paid report' : (context) => FeeReport(type: "OneTime Paid Report",),
      '/paid report' : (context) => FeeReport(type: "Paid Report",),
      '/fine report' : (context) => FeeReport(type: "Fine Report",),
      '/discount report' : (context) => FeeReport(type: "Discount Report",),
      '/payment report' : (context) => FeeReport(type: "Payment Report",),
      '/due fee report' : (context) => FeeReport(type: "Due Fee Report",),
      '/student request' : (context) => StudentsRequests(),
      '/arrange meeting': (context) => Calender(fromCourse: false, ),
